function __print_awaken_functions_help() {
cat <<EOF
Additional AwakenOS functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- awakengerrit:   A Git wrapper that fetches/pushes patch from/to AwakenOS Gerrit Review.
- awakenrebase:   Rebase a Gerrit change and push it again.
- awakenremote:   Add git remote for AwakenOS Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cloremote:       Add git remote for matching CodeLinaro repository.
- githubremote:    Add git remote for AwakenOS Github.
...
function breakfast()
{
    target=$1
    local variant=$2
    source ${ANDROID_BUILD_TOP}/vendor/awaken/vars/aosp_target_release
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            lunch awaken_$target-$aosp_target_release-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/awaken-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.awaken.device | grep -q "$AWAKEN_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $AWAKEN_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function awakenremote()
{
    ...
    git remote rm awaken 2> /dev/null
    ...
    local AWAKEN_USER=$(git config --get review.review.awakenos.org.username)
    if [ -z "$AWAKEN_USER" ]
    then
        git remote add awaken ssh://review.awakenos.org:29418/$PFX$PROJECT
    else
        git remote add awaken ssh://$AWAKEN_USER@review.awakenos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'awaken' created"
}
...
function githubremote()
{
    ...
    git remote add github http://github.com/Project-Awaken/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.awaken.device | grep -q "$AWAKEN_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        awakenremote
        git push awaken HEAD:refs/heads/'$1'
    fi
    '
}

function awakengerrit() {
    ...
    local user=`git config --get review.review.awakenos.org.username`
...
function awakenrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "AwakenOS Gerrit Rebase Usage: "
        echo "      awakenrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.awakenos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.awaken.device | grep -q "$AWAKEN_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $AWAKEN_FIXUP_COMMON_OUT ]; then
